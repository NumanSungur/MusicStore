...
namespace WebUI.Controllers
{
    ...
    public class CategoryController : Controller
    {
        ...
        public IActionResult Index(string GetName, int id)
        {
            ...
            ViewBag.Kategori = db.GetByIdFirst(id).Data;
            ViewBag.AltKategori = db.GetAll(id).Data;
            IList<ProductsDto> products = new List<ProductsDto>();           
            if (Pdb.KategoriyeGoreUrunGetirme(db.GetByIdFirst(id).Data.Id).Data != null)
            {
                foreach (var c in Pdb.KategoriyeGoreUrunGetirme(db.GetByIdFirst(id).Data.Id).Data)
                {
                    products.Add(c);
                }
            }
